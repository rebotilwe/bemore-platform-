...
export interface StepReadyContext {
  ...
  /** Professional Step 4 only — the CV upload either is not required, or, if
   *  the user picked a file, must have completed (storedAs present). The step
   *  file owns this state; we just check that no upload is mid-flight. */
...
export function getMissingItems(
  ...
  for (const q of visible) {
    if (!questionPasses(q, formData[q.id])) {
      // Strip trailing punctuation for cleaner toast copy.
      out.push(q.label.replace(/[?:.]+\s*$/, ''));
    }
  }

  // Step 4 — personal: email + phone universal; CV upload mid-flight blocks.
  if (stepIndex === 4) {
    const p = ctx.personal ?? {};
    ...
    if (ctx.uploadInFlight) out.push('CV upload to finish');
  }
